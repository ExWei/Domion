var Domion = function() {
    ...
    Domion.toParam = function(object) {
        var paramString = "";
    ...
    Domion.get = function (params, callback) {
        Domion.ajax({
            method: "GET",
            url: params.url + "?" + Domion.toParam(params.data)
        }, function(req) {
            callback(req);
        });
    };

    Domion.post = function (params, callback) {
        Domion.ajax({
            method: "POST",
            url: params.url,
            data: Domion.toParam(params.data)
        }, function(req) {
            callback(req);
        });
    };

    Domion.title = function (text) {
        var existTitle = $("title");
        if (existTitle.length === 1) {
            existTitle.innerText = text;
        } else {
